...
"""
完成 mmdet - resnetvd  到 pytorch 的转换
"""
import warnings
...
class ResLayer_(nn2.Sequential):
    """ResLayer to build ResNet style backbone.
...
class BasicBlock_(nn2.Module):
    ...
    def __init__(self,
        ...
        # self.norm1_name, norm1 = build_norm_layer(norm_cfg, planes, postfix=1)
        self.norm1_name, norm1 = 'norm1_name', nn2.BatchNorm2d(planes)
        self.norm2_name, norm2 = 'norm2_name', nn2.BatchNorm2d(planes)
...
class ResNet_(nn2.Module):
    ...
    arch_settings = {
        18: (BasicBlock_, (2, 2, 2, 2)),
        34: (BasicBlock_, (3, 4, 6, 3)),
        50: (Bottleneck, (3, 4, 6, 3)),
        101: (Bottleneck, (3, 4, 23, 3)),
        152: (Bottleneck, (3, 8, 36, 3))
    }
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        ...
        if self.deep_stem:
            ...
            self.conv1 = nn.Conv2d(
                ...
                padding=3,
                bias=False)
            self.norm1_name, norm1 = 'norm1_name', nn.BatchNorm2d(  stem_channels), 
            self.add_module(self.norm1_name, norm1)
            self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
...
if __name__ == '__main__':
    ...
    m2 = ResNetV1c_(
        ...
        style='pytorch',
        contract_dilation=True)
    # 1 load original mmdet model
    m1.init_weights()
    # 2 convert mmdet to torch
    # 2.1 name list of original model
    name_list = []
    for net1, net2   in zip(m1.named_parameters(), m2.named_parameters()):
        name = net1[0]
        param = net1[1]
        name_ = net2[0]
        param_ = net2[1]
